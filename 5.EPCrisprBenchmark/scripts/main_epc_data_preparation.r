...
source("../r_df_management/manipulate_df.r")
...
epc_df <- extend_from_center_thick_df(epc_df, ext_dis)

# Keep only standard chromosomes
epc_df <- keep_standard_chromosomes_df(epc_df, std_chr)
...
pos_gr <- pos_df %>% group_exact_positions_df() %>% GRanges()
neg_notsig_gr <- neg_notsig_df %>% group_exact_positions_df() %>% GRanges()
neg_sigfcpos_gr <- neg_sigfcpos_df %>% group_exact_positions_df() %>% GRanges()
