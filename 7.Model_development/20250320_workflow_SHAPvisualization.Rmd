...
```{r}
K562_C$gr <- CAGEfightR::assignTxType(K562_C$gr, txdb) # 44,743
K562_N$gr <- CAGEfightR::assignTxType(K562_N$gr, txdb) # 116,199
```

```{r}
K562_C <- .GlobalEnv$granges_to_df(K562_C, txdb) # 44,743
K562_N <- .GlobalEnv$granges_to_df(K562_N, txdb) # 116,199
```

```{r}
data_ave_C <- .GlobalEnv$compute_data_ave(K562_C) # 401
data_ave_N <- .GlobalEnv$compute_data_ave(K562_N) # 401
```
