...
mkdir -p /projects/ralab/data/projects/nucCAGE_PRIME_paper/11.LDSC_FANTOM5/ldsc_files/annotation_files

# Define the function that will be run for each BED file
process_bed_file() {
    bed_file=$1
    i=$2
    bim_file="/projects/ralab/data/projects/nucCAGE_PRIME_paper/11.LDSC_FANTOM5/ldsc_files/plink_files/1000G.EUR.hg38.${i}.bim"
    
    # Extract the basename without the extension
    bed_basename=$(basename "$bed_file" ".bed")
    
    # Define output annotation file path
    annot_file="/projects/ralab/data/projects/nucCAGE_PRIME_paper/11.LDSC_FANTOM5/ldsc_files/annotation_files/${bed_basename}.${i}.annot.gz"
...
bed_files=(/projects/ralab/data/projects/nucCAGE_PRIME_paper/0.External_resources/FANTOM5_cellFacet_bedFiles/score_range/*.bed)
