...
divergentLoci <- function(ctss, max_gap=400, win_size=200, inputAssay="counts") {
...
#K562
k562.CTSSs <- readRDS("../1.CTSSs/repSub.combined.k562.CTSSs.rmSingletons.rds")

#Select only relevant samples
samp <- colnames(k562.CTSSs)[c(1,2,3,4,5,6,7,10,11,12,13,14,15,17)]

#k562.DLs <- lapply(colnames(k562.CTSSs),function(x) divergentLociTCsSummit(k562.CTSSs[,x],callingAssay="counts"))
k562.DLs.rmS <- lapply(samp,function(x) divergentLociTCsSummit(k562.CTSSs[,x],callingAssay="counts.noSingletons"))


#saveRDS(k562.DLs, "repSub.k562.summitDLs.rds")
saveRDS(k562.DLs.rmS, "repSub.k562.summitDLs.rmSingletons.rds")


rm(k562.DLs.rmS,k562.CTSSs)


k562.CTSSs <- readRDS("../1.CTSSs/poolSub.combined.k562.CTSSs.rmSingletons.rds")

#k562.DLs <- lapply(colnames(k562.CTSSs),function(x) divergentLociTCsSummit(k562.CTSSs[,x],callingAssay="counts"))
k562.DLs.rmS <- lapply(samp,function(x) divergentLociTCsSummit(k562.CTSSs[,x],callingAssay="counts.noSingletons"))


#saveRDS(k562.DLs, "poolSub.k562.summitDLs.rds")
saveRDS(k562.DLs.rmS, "poolSub.k562.summitDLs.rmSingletons.rds")


#Standard FANTOM5 like DL calling
k562.DLs.fantom5Style <- lapply(c("K562_C","K562_N"),function(x) divergentLoci(k562.CTSSs[,x]))
saveRDS(k562.DLs.fantom5Style, "poolSub.k562.DLs.fantom5Style.rds")
